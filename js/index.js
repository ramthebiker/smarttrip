...
.service('Geolocator', function ($q, $http) {
  ...
  var API_URL = 'https://flightdata.azurewebsites.net/api/iata/';
...
.controller('myCtrl', function ($scope, $timeout, Geolocator, $http) {
		...
		$scope.LoadFlights = function (City, IATACode) {
			$http({ method: "GET", url: 'http://flightdata.azurewebsites.net/api/trip' + $scope.From.split(',')[1] + '/' + IATACode + '/' + $scope.TravelDate }).
						...
						$http({ method: "GET", url: 'https://twitterapi.azurewebsites.net/api/SocialMedia/SocialFeeds/'+ City }).
						then(function (response) {
							$scope.social = response.data;
						});
						
						$http({ method: "GET", url: 'https://weatherapi1.azurewebsites.net/api/weather/'+ City }).
						then(function (response) {
...
$scope.GetNearByAptData = function () {
			$http({ method: "GET", url: 'https://flightdata.azurewebsites.net/api/NearestAirport/'+ $scope.To.split(',')[1] }).
						then(function (response) {
...
$scope.GetSocialFeedsData = function () {
			$http({ method: "GET", url: 'https://twitterapi.azurewebsites.net/api/SocialMedia/SocialFeeds/'+ $scope.To.split(',')[0] }).
						then(function (response) {
							$scope.social = response.data;
						});
		}
		...
		$scope.GetWeatherData = function () {
			$http({ method: "GET", url: 'https://weatherapi1.azurewebsites.net/api/weather/'+ $scope.To.split(',')[0] }).
						then(function (response) {
							$scope.whrForecast = response.data;
...
$scope.GetFlightData = function () {
  ...
  angular.element(document).ready(function () {
      ...
      $http({ method: "GET", url: 'https://flightdata.azurewebsites.net/api/iata/' }).
						then(function (response)
						{
